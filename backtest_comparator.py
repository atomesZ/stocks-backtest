...
class BacktestComparator:
    """
    Compare several Backtest on the same plot
    """
    ...
    def plot(self) -> None:
        ...
        fig.update_layout(
            title="Stock comparison", xaxis_title="Date", yaxis_title="Share value"
        )
        fig.show()
    ...
    def plot_DCA(self, monthly_invested: int) -> None:
        ...
        uninvested_trace = go.Scatter(
            x=dates, y=amounts, mode="lines", name="Uninvested cash"
        )
        ...
        fig.update_layout(
            title="DCA comparison", xaxis_title="Date", yaxis_title="Wallet value"
        )
        fig.show()
